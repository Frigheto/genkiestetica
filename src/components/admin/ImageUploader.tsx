...
import { extrairIdGoogleDrive } from '../../utils/youtubeUtils';
...
export function ImageUploader({ photos, onPhotosChange, maxPhotos = 5 }: ImageUploaderProps) {
  ...
  /**
   * Handle upload de arquivo — envia para Supabase Storage
   */
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of filesToUpload) {
        ...
        const publicUrl = await uploadFoto(compressedFile, 'galeria');
  ...
  const handleAddUrl = () => {
    ...
    // Converter URL do Google Drive para URL direta de imagem
    let finalUrl = urlInput;
    const driveId = extrairIdGoogleDrive(urlInput);
    if (driveId) {
